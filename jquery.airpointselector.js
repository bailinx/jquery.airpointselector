 ...
 *
 * Version        : V1.01
 * Maintained by  : hejia@travelsky.com
		...
		this.selector = function(param){
			...
			$this.unbind('keyup').keyup(function(event) {
				...
				if(!isFunctionalKey && (!ctrl || (ctrl && k == 67) || (ctrl && k == 88)) ){
					...
					$airPointselector.css({"top":offset.top + h,"left":offset.left});
			...
			function matchingRule(cityValue,inputValue){
				inputValue=inputValue.replace(/\\/g,'\\\\').replace("(","\\(").replace(")","\\)")
					...
					.replace("$","\\$").replace("^","\\^").replace("[","\\[").replace("]","\\]")
					.replace("{","\\{").replace("}","\\}").replace("|","\\|").replace("/","\\/");
				if(cityValue.toUpperCase().indexOf(inputValue.toUpperCase())==0
					||cityValue.toUpperCase().match(inputValue.toUpperCase())){
					return true;
				}else{
					return false;
				}
